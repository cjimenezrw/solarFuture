<?php if (!empty($_SESSION['modulos'][$this->sysController]['botones'])) { ?>
    ...
    <div class="dropdown pull-right animated bounceIn">
        ...
        <ul class="dropdown-menu bullet dropdown-menu-right" aria-labelledby="exampleIconsDropdown" role="menu" id="optionsBreadcrumb">
<?php
...
?>
            <?php foreach ($_SESSION['modulos'][$this->sysController]['botones'] as $key => $value) {
                ?>
                <li role="presentation"><a role="button" rol="menuitem"
                    ...
                    switch ($value['sFuncion']) {
                        ...
                        case "txt":
                            ...
                            break;
                        default:
                            $function = $value['sFuncion'] . "({skModulo: '" . $value['skModuloPadre'] . "', url: '/" . DIR_PATH . SYS_PROJECT . '/' . $value['skModuloPrincipal'] . '/' . $value['skModuloPadre'] . '/' . $value['sNombreModulo'] . "/', skComportamiento: '" . $value['skComportamiento'] . "'})";
                            echo 'onclick="' . $function . '"';
                            break;
                    }
                    ?>
                    ><i class="icon <?php echo $value['sIcono']; ?>" aria-hidden="true"></i><?php echo $value['sNombreBoton']; ?></a></li>
                    <?php }//ENDFOREACH ?>
        </ul>
    </div>
<?php
