 ...
 */
Class Ardo_inde_Controller Extends Conf_Model {
    ...
    public function consultar() {
        ...
        $configuraciones['query'] = "SELECT cme.* ,ce.sNombre AS  estatus
                                    FROM cat_archivosDocumentos cme
        ...
        foreach(Conn::fetch_assoc_all($result) AS $row){
            ...
            array_push($data['data'],array(
                'skArchivoDocumento'     => $row['skArchivoDocumento'],
                'sNombre'            => $row['sNombre'],
                'estatus'            => $row['estatus'],
                'dFechaCreacion'     => ($row['dFechaCreacion']) ? date('d/m/Y  H:i:s', strtotime($row['dFechaCreacion'])) : '',
                'menuEmergente'      => parent::menuEmergente($reglas,$row['skArchivoDocumento'])
            ));
        }
        return $data;
    }
